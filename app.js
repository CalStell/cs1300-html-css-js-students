...
const updatePage = async () => {
  ...
  const highestSugar = fruitsArray.filter(
    (fruit) => fruit.nutritions.sugar > 15
  );
  const nameHighestSugar = highestSugar.map((fruitName) => fruitName.name);
  console.log(nameHighestSugar);
  ...
  const exampleAddElement = () => {
    ...
    newElement.innerHTML = nameHighestSugar;
